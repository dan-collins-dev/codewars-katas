/* 
In this Kata, you will implement the Luhn Algorithm, which is used to 
help validate credit card numbers.

Given a positive integer of up to 16 digits, return true if it is a 
valid credit card number, and false if it is not.
...
function validate(n) {
    ...
    for (let num of n.toString()) {
        ccNumberArray.push(parseInt(num));
    }

    ccNumberArray.length % 2 === 0
        ? (ccNumberArray = doubleOdds(ccNumberArray))
        : (ccNumberArray = doubleEvens(ccNumberArray));

    let sum = ccNumberArray.reduce(
